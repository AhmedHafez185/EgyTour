<?php 
  ...
  include 'init.php';
	...
	echo '<input type="hidden" class="sid" value="'.$id.'">';
		echo '<input type="hidden" class="PlaceType" value="Place">';
	...
	$stmt2 = $con->prepare("SELECT sum(rate) as Rate  , count(id) as Num FROM rating WHERE sid = ? and PlaceType='Place' group by sid");
 ...
 progressBar($id,"Place",$row2['Num']);
		echo '<div class="wrapper">
			<div class="comment-wrapper">
				...
				';
		$stmt = $con->prepare("SELECT * from comments where serviceId = ? and PlaceType='Place' ");
		$stmt->execute(array($id));
		$rows = $stmt->fetchAll();
		$count = $stmt->rowCount();
		$page='displayPlace.php?id=' . $id;
		if($count >0 )
		{
