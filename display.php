<?php 
  ...
  include 'init.php';
	...
	echo '<input type="hidden" class="sid" value="'.$id.'">';
			echo '<input type="hidden" class="PlaceType" value="Service">';
	...
	$stmt2 = $con->prepare("SELECT sum(rate) as Rate  , count(id) as Num FROM rating WHERE sid = ? and PlaceType='Service' group by sid");
...
for($i=1; $i<=5; $i++)
{
 ...
 progressBar($id,"Service",$row2['Num']);
		echo '<div class="wrapper">
			<div class="comment-wrapper">
				...
				';
		$stmt = $con->prepare("SELECT * from comments where serviceId = ? and PlaceType='Service' ");
		$stmt->execute(array($id));
		$rows = $stmt->fetchAll();
		$count = $stmt->rowCount();
		$page='display.php?id=' . $id;
		if($count >0 )
		{
