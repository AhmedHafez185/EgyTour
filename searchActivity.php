          ...
          <div class="row">
            <ul>
                   <a href="search.php?id='.$_SESSION['keyword'].'"><li role="presentation">Top</li></a>
                    ...
                    <a href="searchActivity.php?id='.$_SESSION['keyword'].'"><li role="presentation" class="ACT">Activity</li></a>
...
<?php
  ...
  if ($keySearch != "")
  { $perPages = 12;
    $stmt = $con->prepare("SELECT count(aid) as ResultCount from activity  where aName like '%$keySearch%'");
    $stmt->execute();
    $rows = $stmt->fetch();
    ...
    $stmt2 = $con->prepare("SELECT * from activity  where aName like '%$keySearch%' limit $Start,$perPages");
    $stmt2->execute();
     ...
     {     echo '<section class="results">
          ...
          if($count  > 12){
            echo ' <nav aria-label="Page navigation">
                    <ul class="pagination">
                      <li>
                        <a href="searchActivity.php?id='.$_SESSION['keyword'].'&page='.($pag-1) .'" aria-label="Previous">
                      ...
                      echo'<li>
                        <a href="searchActivity.php?id='.$_SESSION['keyword'].'&page='.($pag+1) .'" aria-label="Next">
