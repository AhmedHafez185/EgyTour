        ...
        <div class="container">
          <div class="row">
            <ul>
                    ...
                    <a href="searchPlaces.php?id='.$_SESSION['keyword'].'"><li role="presentation" class="ACT">Tourism Places</li></a>
            </ul>
          </div>
        </div>
      </div> ';
...
<?php
  ...
  if ($keySearch != "")
  { $perPages = 12;
    $stmt = $con->prepare("SELECT count(pid) as ResultCount from place  where PName like '%$keySearch%'");
    $stmt->execute();
    $rows = $stmt->fetch();
    ...
    $stmt2 = $con->prepare("SELECT * from place  where PName like '%$keySearch%' limit $Start,$perPages");
    $stmt2->execute();
     ...
     {     echo '<section class="results">
          ...
          if($count  > 12){
            echo ' <nav aria-label="Page navigation">
                    <ul class="pagination">
                      <li>
                        <a href="searchPlace.php?id='.$_SESSION['keyword'].'&page='.($pag-1) .'" aria-label="Previous">
                          <span aria-hidden="true">&laquo;</span>
                        </a>
                      </li>';
                      for($page=1;$page <= $pages;$page++)
                      {
                        echo '<li><a href="searchPlace.php?id='.$_SESSION['keyword'].'&page='.$page .'">'.$page.'</a></li>';
                      }
                      echo'<li>
                        <a href="searchPlace.php?id='.$_SESSION['keyword'].'&page='.($pag+1) .'" aria-label="Next">
              ...
              foreach ($rows2 as $row2) {
                   echo ' <div class="media col-lg-8 col-md-4">
                                ...
                                <div class="media-body">
                                  <a href="displayPlace.php?id='.$row2["pid"].'">
                                    <h4 class="media-heading">'. $row2["PName"] . '</h4>                       
                                  </a>
                                  <p class="resultCat">'.getPlaceType($row2["pid"]).' Place</p>
                                  <p class="resultCat">'.getCity($row2["PCity"]).'</p>
